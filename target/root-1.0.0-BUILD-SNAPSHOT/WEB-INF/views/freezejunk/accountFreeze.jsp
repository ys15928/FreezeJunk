...
<body>
	...
	<div class="modalcenter modalwrap" id="insmodal">
		...
		<div>
			기존 Youtube Studio에서는 스팸 계정을 차단할 경우 차단 설정 이후의 댓글은<br> 보이지 않게 되지만
			기존에 달았던 댓글은 삭제되는데 약 48시간이 소요됩니다.<br> 즉시 삭제를 원할 경우 유튜버가 직접 하나씩
			삭제를 해야합니다. <br> <br> FreezeJunk를 이용할 경우, 댓글을 드래그 하여 입력하고,<br>
			여러 계정을 한꺼번에 입력할 수 있으며,<br> 계정 차단과 함께 해당 계정이 달았던 댓글들을 인기순 댓글에서 모두
			삭제합니다. <br> <br> * 본인 pc의 기본 브라우저에 유튜브 채널의 계정이 로그인 되어있어야
			합니다. <br> <br> 1. 동영상의 URL 입력<br> 2. 삭제 및 차단하고자 하는 계정의
			댓글을 드래그해 입력<br>(여러 개를 입력할 경우 쉼표로 구분)<br> 3. 구글로그인 버튼을 클릭<br> 4.
			...
			크롤링합니다.<br>(모든 댓글에 대해 삭제를 진행 하는 것이 아닙니다.)<br><br> 삭제될 댓글이 없는 경우
			권한 부여 창이 뜨지 않습니다.<br> 쿠키가 차단되어 있는 브라우저의 경우 로그인 과정에서 에러가 발생합니다.<br>
			쿠키를 허용해 주세요.<br> <a
