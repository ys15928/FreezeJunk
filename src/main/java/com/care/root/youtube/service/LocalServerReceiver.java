...
public final class LocalServerReceiver implements VerificationCodeReceiver {
	 ...
	 * @param port Port to use or {@code -1} to select an unused port
	 */
	LocalServerReceiver(String host, int port, String callbackPath, String successLandingPageUrl,
	...
	/**
	 * HttpServer handler that takes the verifier token passed over from the OAuth
	...
	class CallbackHandler implements HttpHandler {
		...
		private void writeLandingHtml(HttpExchange exchange, Headers headers) throws IOException {
			try (OutputStream os = exchange.getResponseBody()) {
				...
				OutputStreamWriter doc = new OutputStreamWriter(os, "EUC-KR");
				doc.write("<html>");
				doc.write("<head><title>처리 완료</title></head>");
				doc.write("<body>");
				doc.write("<h2>처리 완료되었습니다. 이 창을 닫아주세요.</h2>");
				doc.write("</body>");
				doc.write("</html>\n");
				doc.flush();
			}
